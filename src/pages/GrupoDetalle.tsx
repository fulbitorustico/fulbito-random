...
export default function GrupoDetalle() {
  ...
  return (
    <div>
      <Link to="/grupos" className="mb-4 inline-block text-sm font-medium" style={{ color: 'var(--acc-green)' }}>
        ← Volver a grupos
      </Link>
            ...
            {solicitudes.map((s) => (
              <div key={s.id} className="glass flex items-center gap-3 rounded-2xl px-4 py-2.5">
                ...
                <button
                  ...
                  className="tap rounded-full px-3 py-1.5 text-xs font-semibold text-[color:var(--ink-900)]"
      ...
      <div className="mb-4">
        <div className="mb-2 flex items-center justify-between">
          ...
          <Link to={`/partidos/nuevo?grupo=${grupo.id}`} className="tap text-sm font-semibold" style={{ color: 'var(--acc-green)' }}>
            + Crear
          </Link>
        </div>
        {partidos.length === 0 ? (
      ...
      <form onSubmit={agregarSinRegistrar} className="glass mb-4 flex gap-2 rounded-2xl p-3">
        <input
          ...
          className="flex-1 rounded-xl border-0 bg-white/5 px-3 py-2 text-sm outline-none ring-1 ring-white/10 focus:ring-2"
          style={{ color: 'var(--pitch-900)' }}
        />
        <button
          type="submit"
          disabled={agregando || !nombreNuevo.trim()}
          className="tap rounded-xl px-3 py-2 text-sm font-semibold text-[color:var(--ink-900)] disabled:opacity-40"
          style={{ background: 'var(--paper)' }}
        >
          Sumar
        </button>
      ...
      {linkReclamo && (
        <div className="glass mb-4 rounded-2xl p-4 text-sm" style={{ color: 'var(--pitch-700)' }}>
          ...
          <button onClick={compartirReclamo} className="tap font-semibold" style={{ color: 'var(--acc-green)' }}>
